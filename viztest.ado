...
program viztest, rclass
	...
	mata: pvals = normal(abs(diffs:/diffSE))
	...
	forval l = 2/`nlevs'  {
		...
		mata: LL = LL, newBhat[,1] - invnormal(`qtl'):*sqrt(diagonal(newV))[,1]
		mata: UU = UU, newBhat[,1] + invnormal(`qtl'):*sqrt(diagonal(newV))[,1]
		}
	mata: maxU = colmax(UU)
	mata: minL = colmin(LL)
	...
	forval l = 2/`nlevs'  {
		local qtl = 1-(1-levs[`l',1])/2
		mata: LL = LL, bhat[,1] - invnormal(`qtl'):*sqrt(diagonal(V))[,1]
		mata: UU = UU, bhat[,1] + invnormal(`qtl'):*sqrt(diagonal(V))[,1]
		}
	mata: maxU = colmax(UU)
	mata: minL = colmin(LL)
	mata: L = LL[pairs[,1], ]
	...
	mata: st_local("elev", strofreal(esy[1,1]))
	mata: st_local("slev", strofreal(sml[1,1]))
	mata: st_local("blev", strofreal(bg[1,1]))
	mata: st_local("mlev", strofreal(mid[1,1]))


	di " "
	di "Optimal Levels: "
	di " "
	di "Smallest Level: `slev'"
	di "Middle Level: `mlev'"
	di "Largest Level: `blev'"
	di "Easiest Level: `elev'"
	di " "
	if `nrm' > 1{
	di " "
